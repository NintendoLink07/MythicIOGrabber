...
local function createEntryFrame(applicantID, debug)
	...
	if(addonApplicantList[applicantID]["frame"]) then
		applicantsFrame = addonApplicantList[applicantID]["frame"]
	else
		applicantsFrame = miog.temporaryFramePool:Acquire("BackdropTemplate")
		...
		for index = 1, applicantData.numMembers, 1 do
			...
			if(fullName or debug) then
				...
				if(index == 1 and allowedToInvite or index == 1 and debug) then
					...
					applicantsFrame.declineButton:SetScript("OnClick", function(self, button, down)
					...
					applicantsFrame.inviteButton:SetScript("OnClick", function(self, button, down)
				...
				for dngIndex = 1, 8, 1 do
					...
					if(profile) then --If Raider.IO is installed
						if(mythicKeystoneProfile) then
							...
							if(dngIndex == 1) then
							elseif(dngIndex == 2) then
							elseif(dngIndex == 3) then
							elseif(dngIndex == 4) then
							elseif(dngIndex == 5) then
