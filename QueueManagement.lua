...
local function setPopupInfo(resultID, reason, groupName, activityName, leaderName)
							...
							if(eligible == false) then
								...
								if(reason) then
									...
									if(MIOG_NewSettings.filterPopup and not disabledPopups[v] and not currentPopup and not pendingStatus and not searchResultInfo.isDelisted) then
										...
										setPopupInfo(v, reason[1], searchResultInfo.name, activityInfo.fullName, searchResultInfo.leaderName)
										
										--popupFrame.ButtonPanel.CancelAndSearch:SetAttribute("type", "macro")
										--popupFrame.ButtonPanel.CancelAndSearch:SetAttribute("macrotext1", "/run C_LFGList.CancelApplication(" .. currentPopup .. ")")
...
miog.loadQueueSystem = function()
	...
	popupFrame = CreateFrame("Frame", "MythicIOGrabber_PopupFrame", UIParent, "MIOG_PopupFrame")
	...
	popupFrame.ButtonPanel.CancelAndSearch:SetScript("OnClick", function()
		...
		miog.openSearchPanel(currentCategoryID or LFGListFrame.SearchPanel.categoryID, currentFilters or LFGListFrame.SearchPanel.filters, true)
	end)
	...
	currentCategoryID = 2
	currentFilters = 1
	--setPopupInfo(001, "Fuck you", "+99", "Test Popup", "Rhany")
end
