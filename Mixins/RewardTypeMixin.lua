...
function RewardTypeMixin:OnEnter(childIndex)
    if(self.info) then
        ...
        if(farthestCompletedActivity) then
            ...
            elseif self.info.type == Enum.WeeklyRewardChestThresholdType.Raid then
                if(farthestCompletedActivity.progress < farthestCompletedActivity.threshold) then
                    if farthestCompletedActivity.progress == 0 then
                        GameTooltip_AddNormalLine(GameTooltip, string.format(GREAT_VAULT_REWARDS_RAID_INCOMPLETE, farthestCompletedActivity.threshold - farthestCompletedActivity.progress, self:GetRaidName() or ""));
                    else
                        GameTooltip_AddNormalLine(GameTooltip, string.format(GREAT_VAULT_REWARDS_RAID_INPROGRESS, farthestActivity.threshold - farthestActivity.progress, self:GetRaidName() or ""));
                    end
                end
