...
local diff = diff_main(
'Raszageth sweeps across the platform, exhaling lightning in a line as she travels. Inflicts 110,387 Nature damage instantly and an additional 22,077 Nature damage every 2 sec for 20sec to players standing in the impact area.',
'Raszageth sweeps across the platform, exhaling lightning in a line as she travels. Inflicts 1.7 million Nature damage to players standing in the impact area.')
...
for i,tuple in ipairs(diff) do
  ...
  currentOperator = tuple[1]

  if(currentOperator == 0) then
    base = base .. tuple[2]

  else
    if(currentOperator == -1) then
        diff1 = diff1 .. string.sub(diff[i-1][2], strlen(diff[i-1][2])) .. tuple[2]
        base = string.sub(base, 0, strlen(base) - 1) .. "[1]"

    else
        diff2 = diff2 .. string.sub(diff[i-2][2], strlen(diff[i-2][2])) .. tuple[2]
